...
class NotificationAdapter:
    ...
    async def send_plate_notifications(
        plate: str, title: str, body: str, **kwargs
    ) -> Dict[str, Any]:
        """
        为指定车牌发送推送通知
        ...
        try:
            ...
            if not plate_config:
                ...
                return {"success": False, "error": error_msg, "plate": plate}

            # 发送推送
            result = await unified_pusher.push(
                plate_config=plate_config, title=title, body=body, **kwargs
            )
    ...
    @staticmethod
    async def send_all_notifications(title: str, body: str, **kwargs) -> Dict[str, Any]:
        """
        向所有配置的车牌发送推送通知
