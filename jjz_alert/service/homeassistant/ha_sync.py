...
class HomeAssistantSyncService:
    ...
    @with_error_handling(
        exceptions=(APIError, NetworkError, HomeAssistantAPIError, Exception),
        service_name="ha_sync_service",
        default_return={"success": False, "error": "连接测试失败"},
    )
    async def test_connection(self) -> Dict[str, Any]:
        """测试Home Assistant连接"""
        try:
            if not self.config.enabled:
                return {"success": False, "error": "Home Assistant集成未启用"}

            client = await self._get_client()
            if not client:
                return {"success": False, "error": "无法创建HA客户端"}

            result = await client.test_connection()

            if result["success"]:
                logging.debug(
                    f"Home Assistant连接测试成功: {result.get('version', 'unknown')}"
                )
            else:
                logging.error(
                    f"Home Assistant连接测试失败: {result.get('error', 'unknown')}"
                )
        ...
        try:
            ...
            for attempt in range(self.config.retry_count):
                try:
                    ...
                    sync_result.update(
                        {
                            ...
                            "failed_entities": result["total_count"]
                    ...
                    if "Authentication failed" in str(e) or "Access forbidden" in str(
                        e
                    ):
                        # 认证错误不重试
            ...
            if sync_result["success"]:
                ...
                logging.error(
                    f"车牌 {plate_number} HA同步失败: {sync_result.get('error', 'unknown')}"
                )
    ...
    async def sync_multiple_plates(
        self, plates_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """批量同步多个车牌数据"""
        ...
        try:
            ...
            # 记录批量同步结果
            success_rate = (
                (batch_result["success_plates"] / batch_result["total_plates"] * 100)
                if batch_result["total_plates"] > 0
                else 0
            )
    ...
    async def cleanup_stale_entities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            client = await self._get_client()
            if not client:
                return {"success": False, "error": "无法获取HA客户端"}
...
async def sync_to_homeassistant(
    jjz_results: Dict[str, JJZStatus], traffic_results: Dict[str, PlateTrafficStatus]
