...
class HomeAssistantClient:
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        ...
        if self._session is None or self._session.closed:
            ...
            self._session = aiohttp.ClientSession(
                headers=self.headers, raise_for_status=False
            )
    ...
    async def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not version:
                version = response.get("ha_version") or response.get(
                    "version_info", {}
                ).get("version")
            ...
            return {
                ...
                "version": version
                or "connected",  # 如果确实没有版本信息，显示connected
            }
    ...
    async def set_entity_state(
        ...
        """设置实体状态"""
        data = {"state": str(state), "attributes": attributes or {}}
    ...
    async def register_device(self, device_info: HADeviceInfo) -> bool:
        ...
        safe_identifiers = re.sub(
            r"[^a-z0-9_]", "_", str(device_info.identifiers).lower()
        )
        ...
        try:
            success = await self.set_entity_state(
                temp_entity_id, "registered", attributes
            )
    ...
    async def sync_entity_state(self, entity_state: HAEntityState) -> bool:
        ...
        try:
            ...
            success = await self.set_entity_state(
                entity_state.entity_id, entity_state.state, attributes
            )

            if success:
                logging.debug(
                    f"实体状态同步成功: {entity_state.entity_id} = {entity_state.state}"
                )
            else:
    ...
    async def sync_plate_device(self, plate_device: HAPlateDevice) -> Dict[str, Any]:
        ...
        try:
            ...
            # 记录同步结果
            success_rate = (
                (sync_results["success_count"] / sync_results["total_count"] * 100)
                if sync_results["total_count"] > 0
                else 0
            )
    ...
    async def cleanup_stale_entities(self, current_plates: List[str]) -> Dict[str, Any]:
        """清理过期的实体（不在当前车牌列表中的）"""
        cleanup_results = {"deleted_count": 0, "total_found": 0, "errors": []}
        ...
        try:
            ...
            for entity in all_entities:
                ...
                # 检查是否属于当前车牌
                is_current = any(
                    entity_id.startswith(prefix) for prefix in current_prefixes
                )
                if not is_current:
            ...
            if entities_to_delete:
                ...
                delete_tasks = [
                    self.delete_entity(entity_id) for entity_id in entities_to_delete
                ]
            ...
            logging.info(
                f"实体清理完成: 删除 {cleanup_results['deleted_count']} 个过期实体"
            )
