...
class HAPlateDevice:
    ...
    def get_device_info(
        self, manufacturer: str = "JJZ Alert", model: str = "Beijing Vehicle"
    ) -> HADeviceInfo:
        """获取设备信息"""
        # 使用拼音转换后的车牌号作为设备标识符
    ...
    def get_combined_sensor_state(
        ...
        """获取合并的进京证和限行传感器状态"""
        # 提取省份信息和车牌剩余部分
        province_chinese, province_pinyin = extract_province_from_plate(
            self.plate_number
        )
        ...
        attributes = {
            ...
            "jjz_valid_start": self._format_valid_date(
                self.jjz_valid_start, self.jjz_valid_end
            )[0],
            "jjz_valid_end": self._format_valid_date(
                self.jjz_valid_start, self.jjz_valid_end
            )[1],
            ...
            "traffic_limited_today_text": (
                "限行" if self.traffic_limited_today else "不限行"
            ),
    ...
    def from_jjz_and_traffic_data(
        ...
        return cls(
            ...
            traffic_rule_desc=traffic_status_data.get("rule", {}).get(
                "limited_numbers", "未知"
            ),
            traffic_limited_tail_numbers=traffic_status_data.get("rule", {}).get(
                "limited_numbers", "0"
            ),
            # 元信息
