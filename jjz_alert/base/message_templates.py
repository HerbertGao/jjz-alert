...
class MessageTemplateManager:
    ...
    def _get_default_templates(self) -> Dict[str, str]:
        ...
        return {
            ...
            # 过期状态模板
            "expired_status": (
                "车牌 ${display_name} 的进京证 已过期，请及时续办。" "${sycs_part}"
            ),
            # 审核中状态模板
            "pending_status": (
    ...
    def format_valid_status(
        ...
        try:
            # 处理六环内剩余次数部分
            if sycs:
                sycs_part = Template(self.templates["sycs_part"]).safe_substitute(
                    sycs=sycs
                )
            else:
    ...
    def format_pending_status(
        self, display_name: str, jjz_type: str, apply_time: str
    ) -> str:
        """
        格式化审核中状态消息
...
def initialize_templates_from_config(config_manager=None):
    ...
    try:
        if config_manager is None:
            from jjz_alert.config.config import config_manager as cm
        ...
        if new_templates:
            ...
            logging.getLogger(__name__).info(
                f"已从配置文件加载 {len(new_templates)} 个自定义模板"
            )
        else:
