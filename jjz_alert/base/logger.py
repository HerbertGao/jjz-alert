...
使用方法：只需在程序入口或模块顶部 `import jjz_alert.base.logger`，即可完成全局初始化。
"""
...
class StructuredLogger:
    ...
    def log_structured(
        ...
        # 记录日志
        self.logger.log(
            level, f"STRUCTURED: {json.dumps(structured_data, ensure_ascii=False)}"
        )
    ...
    def log_business_operation(
        ...
        """记录业务操作日志"""
        business_data = {"success": success, "operation": operation}
    ...
    def log_performance(
        ...
        """记录性能日志"""
        perf_data = {"duration_ms": duration_ms, "success": success}
