...
async def main():
        ...
        try:
            if not send_next_day:
                ...
                if tomorrow_statuses:
                    ...
                    push_result = await push_jjz_status(plate_cfg, tomorrow_data, target_date=tomorrow_date,
                                                        is_next_day=True)
                else:
                    # 无次日记录，需要提醒办理
    ...
    # 创建并发任务
    plate_tasks = [process_plate_async(plate_cfg) for plate_cfg in plate_configs if
                   plate_cfg.plate in plate_to_statuses]
