...
class HomeAssistantClient:
    ...
    async def sync_plate_device(self, plate_device: HAPlateDevice) -> Dict[str, Any]:
        ...
        try:
            ...
            # 记录同步结果
            success_rate = (sync_results['success_count'] / sync_results['total_count'] * 100) if sync_results['total_count'] > 0 else 0
            
            if sync_results['success_count'] == sync_results['total_count']:
                logging.info(f"车牌设备 {plate_device.plate_number} 同步成功: {sync_results['success_count']}/{sync_results['total_count']} 实体")
            else:
                logging.warning(f"车牌设备 {plate_device.plate_number} 部分同步失败: {sync_results['success_count']}/{sync_results['total_count']} 实体成功 ({success_rate:.1f}%)")
