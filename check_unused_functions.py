...
class FunctionUsageChecker(ast.NodeVisitor):
    ...
    def __init__(self, file_path: str):
        ...
        self.defined_methods: Dict[str, Set[str]] = defaultdict(set)  # class_name -> methods
        self.called_functions: Set[str] = set()
        self.called_methods: Dict[str, Set[str]] = defaultdict(set)  # class_name -> methods
        self.current_class: str = None
        self.imports: Dict[str, str] = {}  # imported_name -> module_name
        self.variable_types: Dict[str, str] = {}  # variable_name -> class_name (用于追踪实例类型)
...
def get_python_files(root_dir: Path, exclude_tests: bool = True) -> List[Path]:
    """获取所有Python文件"""
    python_files = []
    exclude_dirs = {".venv", "venv", "env", "__pycache__", ".git", "htmlcov", "node_modules"}
...
def get_function_qualname(file_path: Path, func_name: str, class_name: str = None, base_dir: Path = None) -> str:
    """获取函数的完整限定名"""
    # 计算相对于基础目录的模块路径
    if base_dir:
...
def check_unused_functions():
    ...
    all_defined_functions: Dict[str, Tuple[Path, str, str]] = {}  # qualname -> (file, func_name, class_name)
    ...
    for file_path in python_files:
        ...
        if checker:
            ...
            for func_name in checker.defined_functions:
                qualname = get_function_qualname(file_path, func_name, base_dir=target_dir)
                all_defined_functions[qualname] = (file_path, func_name, None)
            
            # 收集所有定义的类方法
            for class_name, methods in checker.defined_methods.items():
                for method_name in methods:
                    qualname = get_function_qualname(file_path, method_name, class_name, base_dir=target_dir)
                    all_defined_functions[qualname] = (file_path, method_name, class_name)
    
    # 构建全局变量类型映射（跨文件）
    global_variable_types: Dict[str, Dict[str, str]] = {}  # file_path -> {var_name -> class_name}
                ...
                for other_file, other_checker in all_checkers.items():
                    if var_class in other_checker.defined_classes:
                        ...
                        if var_name in checker.called_methods:
                            all_called_methods[var_class].update(checker.called_methods[var_name])
                ...
                if other_file and other_checker:
                    ...
                    if imported_name in other_checker.variable_types:
                        ...
                        if imported_name in checker.called_methods:
                            all_called_methods[var_class].update(checker.called_methods[imported_name])
    ...
    for qualname, (file_path, func_name, class_name) in all_defined_functions.items():
        ...
        if not is_used and class_name:
            if class_name in all_called_methods and func_name in all_called_methods[class_name]:
                is_used = True
                            ...
                            for i, line in enumerate(lines):
                                if '__all__' in line:
                                    ...
                                    for j in range(i, min(i + 20, len(lines))):
                                        if func_name in lines[j] and ('"' in lines[j] or "'" in lines[j]):
                                            in_all = True
                                            break
                                    break
                            if in_all:
